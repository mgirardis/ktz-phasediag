...
subroutine strSplit(str,delims,args,nargs)
...
! adicionado por Mauricio
function strToFloat(str) result(r)
    character(len=*) :: str
    real*4 :: r
    integer :: ilen, ipos
    ...
    if (.not.is_digit(str(ilen:ilen)) .and. ipos /= 0) then
        write (*,*) 'error converting number ', str
        r = 0.0D0
        return
    end if
    read(str,*) r
...
function strToDouble(str) result(r)
    character(len=*) :: str
    real*8 :: r
    integer :: ilen, ipos
...
! adicionado por Mauricio
function strToInteger(str) result (r)
    character(len=*) :: str
    integer :: r
    real*8 :: rTemp

    rTemp = strToDouble(str)
    if (abs(rTemp) > huge(r)) then
        write (*,*) 'error converting number ',str
        r = 0
        return
    end if
